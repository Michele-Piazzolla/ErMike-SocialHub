...
//json per chat messaggiustica tra utenti 1:1
const messagesFile = './data/messages.json';
if (!fs.existsSync(messagesFile)) fs.writeFileSync(messagesFile, '[]');

//arrey json per la gestione follow e follwers
const followsFile = './data/follows.json';
if (!fs.existsSync(followsFile)) fs.writeFileSync(followsFile, '[]');
...
// Profilo pubblico di un utente (qualsiasi)
app.get('/profile/:username', requireLogin, (req, res) => {
  ...
  // Rimuoviamo la password prima di passare alla view
  const safeUser = {
  ...
  // Post dell'utente
  const posts = readJSON(FILES.posts)
    .filter(p => p.username === targetUsername)
    .sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));

  // Conta follower e seguiti
  const follows = readJSON(followsFile);
  const followerCount = follows.filter(f => f.followed === targetUsername).length;
  const followingCount = follows.filter(f => f.follower === targetUsername).length;

  // L'utente loggato segue questo profilo?
  const currentUser = req.session.user.username;
  const isFollowing = follows.some(f => f.follower === currentUser && f.followed === targetUsername);
  const isOwner = (currentUser === targetUsername);

  res.render('profilePublic', {
    user: req.session.user,        // utente loggato (per navbar)
    profileUser: safeUser,         // utente visualizzato
    posts,
    followerCount,
    followingCount,
    isOwner,
...
app.get('/private-chat', requireLogin, (req, res) => {
  res.render('privateChat', { user: req.session.user });
});

// Restituisce la lista di utenti con cui ho avuto conversazioni
app.get('/api/conversations', requireLogin, (req, res) => {
  ...
  messages.forEach(msg => {
    if (msg.from === currentUser) usersSet.add(msg.to);
    if (msg.to === currentUser) usersSet.add(msg.from);
  });

  const users = Array.from(usersSet).map(username => {
    // Recupera avatar e fullName da users.json
    const userRecord = readJSON(FILES.users).find(u => u.username === username);
    return {
      username,
...
// Restituisce i messaggi tra me e un altro utente
app.get('/api/messages/:withUser', requireLogin, (req, res) => {
  const currentUser = req.session.user.username;
  const withUser = req.params.withUser;
  const messages = readJSON(messagesFile);
  const chat = messages.filter(m =>
    (m.from === currentUser && m.to === withUser) ||
...
app.post('/follow/:username', requireLogin, (req, res) => {
  ...
  let follows = readJSON(followsFile);
  const alreadyFollowing = follows.some(f => f.follower === follower && f.followed === followed);

  if (alreadyFollowing) {
    // Unfollow: rimuovi la coppia
...
// =============================================================
//  WEBSOCKET — Chat globale + messaggi privati
// =============================================================
const wss = new Server({ server });
const connessi = new Map();          // ws → username
let storico = readJSON(FILES.chat);  // storico globale in RAM
...
wss.on('connection', ws => {
  ws.on('message', raw => {
    ...
    if (dati.tipo === 0) {
      ...
      ws.send(JSON.stringify({ tipo: 1, storico }));  // storico chat globale
      return;
    }

    // ── TIPO 1: messaggio globale ─────────────────────────
    if (dati.tipo === 1) {
    ...
    // ── TIPO 10: messaggio privato ────────────────────────
    if (dati.tipo === 10) {
      const from = connessi.get(ws);
      if (!from) return;
      ...
      for (const [sock, username] of connessi) {
        if (username === to) {
      ...
      const pacchetto = JSON.stringify({ tipo: 10, msg });
      if (recipientSocket && recipientSocket.readyState === WebSocket.OPEN) {
        recipientSocket.send(pacchetto);
      }
